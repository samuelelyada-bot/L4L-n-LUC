...
with g_col1:
    with st.expander("📋 1. Lot-for-Lot (L4L)", expanded=False):
        st.markdown("""<div class='text-justify'><b>Concept:</b> Orders exact net requirements per discrete period.<br><br><b>Function:</b> Minimizes holding values straight to a zero-point baseline.</div>""", unsafe_allow_html=True)
    with st.expander("🚀 6. Silver-Meal (SM)", expanded=False):
        st.markdown("""<div class='text-justify'><b>Concept:</b> Minimizes average total cost per period by incrementally adding future demands.<br><br><b>Function:</b> Excellent for highly volatile or variable demand patterns.</div>""", unsafe_allow_html=True)

with g_col2:
    with st.expander("🎯 2. Economic Order Quantity (EOQ)", expanded=False):
        st.markdown("""<div class='text-justify'><b>Concept:</b> Establishes a fixed structural lot sizing standard to balance ordering and holding profiles.<br><br><b>Function:</b> Maximizes efficiency when cross-horizon demands are linear.</div>""", unsafe_allow_html=True)
    with st.expander("⏱️ 7. Period Order Quantity (POQ)", expanded=False):
        st.markdown("""<div class='text-justify'><b>Concept:</b> Converts classical EOQ lot intervals into a time-based coverage integer.<br><br><b>Function:</b> Reduces holding spikes by stabilizing batch frequencies.</div>""", unsafe_allow_html=True)

with g_col3:
    with st.expander("🔍 3. Least Unit Cost (LUC)", expanded=False):
        st.markdown("""<div class='text-justify'><b>Concept:</b> Aggregates upcoming period blocks until total cost per unit starts to climb.<br><br><b>Function:</b> Controls cost variances dynamic to changing horizon shifts.</div>""", unsafe_allow_html=True)
    with st.expander("🔒 8. Fixed Order Quantity (FOQ)", expanded=False):
        st.markdown("""<div class='text-justify'><b>Concept:</b> Locks a rigid, predetermined supplier multiplier lot whenever net demands break safety lines.<br><br><b>Function:</b> Fits industrial environments bound by strict box, pallet, or shipping container sizing.</div>""", unsafe_allow_html=True)

with g_col4:
    with st.expander("⚖️ 4. Part Period Balancing (PPB)", expanded=False):
        st.markdown("""<div class='text-justify'><b>Concept:</b> Synchronizes setup costs against cumulative holding components by locating the closest fit to EPP.<br><br><b>Function:</b> Drives balanced material cost optimization.</div>""", unsafe_allow_html=True)
    with st.expander("🧱 9. Minimum Order Quantity (MOQ)", expanded=False):
        st.markdown("""<div class='text-justify'><b>Concept:</b> Guarantees that every placed order dynamically scales up to at least hit a supplier's critical threshold.<br><br><b>Function:</b> Satisfies non-negotiable vendor manufacturing limits.</div>""", unsafe_allow_html=True)

with g_col5:
    with st.expander("💸 5. Least Total Cost (LTC)", expanded=False):
        st.markdown("""<div class='text-justify'><b>Concept:</b> Accumulates periods sequentially and cuts off immediately the moment total holding cost matches or exceeds setup cost.<br><br><b>Function:</b> Practical heuristic targeting structural balance without distance comparison.</div>""", unsafe_allow_html=True)
    with st.expander("🔬 10. Wagner-Whitin (WW)", expanded=False):
        st.markdown("""<div class='text-justify'><b>Concept:</b> Dynamic programming model that evaluates all possible order combinations across the horizon.<br><br><b>Function:</b> Guarantees the absolute mathematically optimal global minimum cost.</div>""", unsafe_allow_html=True)
...
if df_workbench is not None and not df_workbench.empty:
    ...
    def calculate_multi_mrp(demands, s_receipts, setup, hold, init_inv, ss, lt, f_lot, m_qty):
        ...
        # 🔥 10. WAGNER-WHITIN (WW) — Dynamic Programming
        INF = float('inf')
        f = [INF] * (n + 1)
        order_at = [0] * (n + 1)
        f[0] = 0
        ...
        # Build Table DP Evaluation Matrix Rows
        dp_rows = []
        for j in range(1, n + 1):
            for i in range(1, j + 1):
                holding = sum(net_req[k-1] * hold * (k - i) for k in range(i, j + 1))
                cost = f[i-1] + setup + holding
                dp_rows.append({
                    'Order Period': period_labels[i-1],
                    'Covers Until': period_labels[j-1],
                    'Cumulative Holding': holding,
                    'Evaluated Cost': cost,
                    'Selected': '✅' if (order_at[j] == i and cost == f[j]) else ''
                })
        df_ww_dp_table = pd.DataFrame(dp_rows)
        ...
        return {
            ...
            'ww': {'poh': ww_poh, 'rec': ww_rec, 'rel': ww_rel, 'setup': c_ww_setup, 'hold': c_ww_hold, 'total': c_ww_setup + c_ww_hold, 'dp_table': df_ww_dp_table}
        }
    ...
    with tabs_list[1]:
        ...
        if holding_cost == 0:
            ...
            with st.expander("🔬 CLICK HERE TO VIEW FORMULA LOG CALCULATIONS (EOQ)"):
                st.markdown(f"$$EOQ = \\sqrt{{\\frac{{2 \\times {res['eoq']['avg_demand_gross']:.4f} \\times {setup_cost:,.2f}}}{{{holding_cost:,.2f}}}}}$$")
    ...
    with tabs_list[5]:
        ...
        if res['poq']['interval'] == 1:
            st.warning("⚠️ POQ Interval is 1 period. The result will be identical to Lot-for-Lot (L4L).")
    ...
    # 🔥 TAB 9: LEAST TOTAL COST (LTC)
    with tabs_list[8]:
        st.subheader("💸 Least Total Cost (LTC) Sequential Cut-off Grid")
        st.markdown("##### Dynamic Lot Search Log Blocks Trace (Stops immediately when Holding ≥ Setup):")
    ...
    with tabs_list[9]:
        ...
        st.success("🎯 **Solusi Optimal Global:** Ini adalah metode eksak yang menjamin total biaya minimum mutlak di seluruh horison perencanaan.")
        
        with st.expander("🔬 CLICK HERE TO VIEW WAGNER-WHITIN DP COST EVALUATION TABLE"):
            fmt_ww = {'Cumulative Holding': '{:.2f}', 'Evaluated Cost': '{:.2f}'}
            st.dataframe(res['ww']['dp_table'].style.format(fmt_ww), hide_index=True, use_container_width=True)
    ...
    # Grid dynamic layout adjustment for 10 cards
    ...
    for idx, key in enumerate(m_keys):
        ...
        with col_target:
            ...
            st.markdown(f"""
                <div style='background-color: #f4efdc; padding: 14px; border-radius: 8px; border-left: 5px solid #6a0708; margin-bottom: 15px; min-height: 130px;'>
            ...
            """, unsafe_allow_html=True)
            if key == 'WW':
                st.caption("🔬 Exact optimal (Dynamic Programming)")
