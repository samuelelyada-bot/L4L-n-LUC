...
# ==========================================
st.set_page_config(page_title="MRP Calculator - Multi-Metode DSS", layout="wide")
st.title("📦 Aplikasi Perencanaan Kebutuhan Material (MRP) - Multi-Metode")
st.caption("Edisi DSS Lengkap: Perbandingan L4L, LUC, EOQ, dan Part Period Balancing (PPB)")
st.markdown("---")
...
if df_kerja is not None and not df_kerja.empty:
    ...
    # ==========================================
    # 5. HALAMAN UTAMA - DASHBOARD SUMMARY MATRIX
    # ==========================================
    st.markdown("---")
    st.header("🏁 Hasil Komparasi Performa Multi-Metode")
    
    # Kumpulkan total biaya untuk dicari yang termurah
    biaya_dict = {'Lot-for-Lot (L4L)': res['l4l']['total'], 'Least Unit Cost (LUC)': res['luc']['total'], 'Economic Order Quantity (EOQ)': res['eoq']['total'], 'Part Period Balancing (PPB)': res['ppb']['total']}
    best_method = min(biaya_dict, key=biaya_dict.get)
    
    # Tampilkan Grid Rekap Biaya
    m1, m2, m3, m4 = st.columns(4)
    with m1:
        st.markdown(f"<div style='background-color: #f8f9fa; padding: 15px; border-radius: 8px; border-left: 5px solid #FF6B6B;'><strong>L4L Cost</strong><br><span style='font-size: 22px; font-weight:bold;'>Rp {res['l4l']['total']:,.0f}</span></div>", unsafe_allow_html=True)
    with m2:
        st.markdown(f"<div style='background-color: #f8f9fa; padding: 15px; border-radius: 8px; border-left: 5px solid #4D96FF;'><strong>LUC Cost</strong><br><span style='font-size: 22px; font-weight:bold;'>Rp {res['luc']['total']:,.0f}</span></div>", unsafe_allow_html=True)
    with m3:
        st.markdown(f"<div style='background-color: #f8f9fa; padding: 15px; border-radius: 8px; border-left: 5px solid #6BCB77;'><strong>EOQ Cost</strong><br><span style='font-size: 22px; font-weight:bold;'>Rp {res['eoq']['total']:,.0f}</span><br><small>Size: {res['eoq']['size']} unit</small></div>", unsafe_allow_html=True)
    with m4:
        st.markdown(f"<div style='background-color: #f8f9fa; padding: 15px; border-radius: 8px; border-left: 5px solid #f9d949;'><strong>PPB Cost</strong><br><span style='font-size: 22px; font-weight:bold;'>Rp {res['ppb']['total']:,.0f}</span></div>", unsafe_allow_html=True)

    st.success(f"🏆 **Rekomendasi Keputusan:** Metode **{best_method}** menghasilkan total efisiensi logistik paling optimal berdasarkan parameter saat ini.")

    # TABS DETAIL PER METODE
    ...
    def tampilkan_tabel_mrp(nama_metode, data_dict, max_cap):
        ...
        if max(data_dict['poh']) > max_cap:
            st.warning(f"⚠️ **Kapasitas Terlampaui:** Stok pengaman + akumulasi lot pada {nama_metode} melebihi limit gudang ({max_cap} unit).")
    ...
    with t_luc:
        ...
        with st.expander("🔬 LOG ITERASI PERHITUNGAN DETAIL (LUC)"):
            for idx, df_iter in enumerate(res['luc']['iters']):
                st.markdown(f"**Langkah Pembentukan Lot Ke-{idx+1}:**")
                st.dataframe(df_iter.style.format({'Biaya Pesan': '{:.2f}', 'Biaya Simpan': '{:.2f}', 'Total Biaya': '{:.2f}', 'LUC (Cost/Unit)': '{:.4f}'}), hide_index=True, use_container_width=True)
    ...
    with t_ppb:
        ...
        with st.expander("🔬 LOG ITERASI KESEIMBANGAN PART PERIOD (PPB)"):
            for idx, df_iter in enumerate(res['ppb']['iters']):
                st.markdown(f"**Langkah Pembentukan Lot Ke-{idx+1}:**")
                st.dataframe(df_iter.style.format({'Biaya Pesan': '{:.2f}', 'Biaya Simpan': '{:.2f}', 'Selisih |S-H|': '{:.2f}'}), hide_index=True, use_container_width=True)
    ...
    st.info("💡 Masukkan data kebutuhan untuk menjalankan komparasi DSS.")
